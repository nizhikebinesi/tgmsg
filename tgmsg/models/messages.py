...
class DocumentMessage(Message):
    def __init__(self, document: str, parse_mode: str = None, reply_markup: Keyboard = None,
                 disable_content_type_detection: bool = None, disable_notification: bool = None,
                 reply_to_message_id: int = None):
        # TODO: add using a document as InputFile posibility
        #  https://core.telegram.org/bots/api#senddocument
        if not isinstance(document, str):
            raise TypeError('document must be an instance of str')
        if parse_mode is not None:
            if not isinstance(parse_mode, str):
                raise TypeError('parse_mode must be an instance of str')
        if reply_markup is not None:
        ...
        if disable_content_type_detection is not None:
            if not isinstance(disable_content_type_detection, bool):
                raise TypeError('disable_content_type_detection must be an instance of bool')
        if disable_notification is not None:
            if not isinstance(disable_notification, bool):
                raise TypeError('disable_notification must be an instance of bool')
        if reply_to_message_id is not None:
    ...
    def to_dict(self):
        ...
        if self.disable_content_type_detection is not None:
            res['disable_content_type_detection'] = self.disable_content_type_detection
        if self.disable_notification is not None:
            res['disable_notification'] = self.disable_notification
        if self.reply_to_message_id is not None:
            res['reply_to_message_id'] = self.reply_to_message_id
